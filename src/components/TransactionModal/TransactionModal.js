    ...
    // } else if(window.wallet._ethersType !== 'Signer') {
      ...
      screenContent = (
        <Modal.Body style={{padding: '15px'}}>
          ...
          {this.state.hash ? <p>You can view your transaction on <a href={`https://${process.env.network === 'homestead' ? '' : 'kovan.' }etherscan.io/tx/${this.state.hash}`} style={{color: 'black', textDecoration: 'underline'}} target="_blank" rel="noopener noreferrer">EtherScan</a></p> : null}
