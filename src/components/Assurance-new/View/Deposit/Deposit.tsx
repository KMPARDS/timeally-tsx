...
class Deposit extends Component<Props & RouteComponentProps<RouteParams>, State> {
  ...
  componentDidMount= async() => {
    if(window.wallet) {
      ...
      const sipPromise = window.tsgapLiquidInstance.functions.sips(window.wallet.address, this.props.match.params.id);
        ...
        if(+event.target.value) {
          if(isDepositAtleastMinimum) {
            if(isLiquidAvailable) {
              insufficientBalanceText = `You can use either your liquid tokens (${lessDecimals(this.state.userLiquidEsBalance)} ES) for this SIP.`;
            } else {
              insufficientBalance = true;
              insufficientBalanceText = `Insufficient ES balance. You only have ${lessDecimals(this.state.userLiquidEsBalance)} liquid ES tokens`;
            }
          } else {
            insufficientBalance = true;
            insufficientBalanceText = `Your amount should be at least ${lessDecimals(this.state.monthlyCommitmentAmount)} ES. `
          }
        }
  ...
  render() {
    ...
    if(this.state.currentScreen === 0) {
      ...
      if(this.state.isLiquidAvailable) {
        displayText = <p>This dApp just noticed that you have <strong>{lessDecimals(this.state.userLiquidEsBalance)} liquid ES tokens</strong>.</p>;
      } else {
        displayText = <p>Seems that you don't have enough ES tokens for making deposit of <strong>{this.state.userAmount} ES</strong> for {getOrdinalString(Number(Number(this.props.match.params.month)) )} Month. Your liquid balance is <strong>{lessDecimals(this.state.userLiquidEsBalance)} ES</strong>. Are you sure you want to proceed? You can get ES tokens from anyone who has ES tokens. ES tokens are also trading on Probit Exchange, where you can exchange your other crypto assets with the exchange community for ES.</p>;
      }
      ...
      screen = (
        <>
          <Card>
            <div className="custom-width" style={{border: '1px solid rgba(0,0,0,.125)', borderRadius: '.25rem', padding:'20px 40px', margin: '15px auto'}}>
              <h3 style={{marginBottom: '15px'}}>{getOrdinalString(Number(this.props.match.params.month))} Monthly Deposit confirmed!</h3>
    ...
    return (
      <Layout
        ...
        //   return x;
        // })()]}
        title={`Deposit ${getOrdinalString(Number(this.props.match.params.month))} Monthly Installment`}
      >
        {screen}
        <TransactionModal
