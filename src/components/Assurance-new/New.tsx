  ...
  render() {
    ...
    if(this.state.currentScreen === 0) {
      ...
      if(this.state.isLiquidAvailable) {
        ...
        displayText = <p>Seems that you don't have enough ES tokens for New SIP of <strong>{this.state.userAmount} ES</strong>. Your liquid balance is <strong>{lessDecimals(this.state.userLiquidEsBalance)} ES</strong>. Are you sure you want to proceed? You can get ES tokens from anyone who has ES tokens. ES tokens are also trading on Probit Exchange, where you can exchange your other crypto assets with the exchange community for ES.</p>;
      }
        ...
        <>
          <Card>
            <div className="custom-width" style={{border: '1px solid rgba(0,0,0,.125)', borderRadius: '.25rem', padding:'20px 40px', margin: '15px auto'}}>
              ...
              <Alert variant="success">Your SIP is initiated. You can view your transaction on <a style={{color: 'black'}} href={`https://eraswap.info/txn/${this.state.txHash}`} target="_blank" rel="noopener noreferrer">Eraswap.info</a></Alert>
